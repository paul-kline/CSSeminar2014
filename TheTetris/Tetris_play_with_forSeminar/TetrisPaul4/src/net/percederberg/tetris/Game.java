...
public class Game extends Object {
    ...
    protected PlacementDecider placementDecider = new PlacementDecider();
    /**
     * The figures used on both futureBoards. All figures are reutilized in
     * order to avoid creating new objects while the game is running. Special
     * care has to be taken when the preview figure and the current figure
    ...
    /**
     * Handles a game start event. Both the main and preview square futureBoards
     * will be reset, and all other game parameters will be reset. Finally the
     * game thread will be launched.
     */
...
//        System.out.println("handling start!");
    ...
    public void handleGameOver() {
        ...
        String floc = ".\\PetersburgGameResult" + tw.GetTimeStamp(true, true, true, true, true, true) + ".txt";
        PrintGameResultHere( tw, floc);
    ...
    public void SetAutomatic(boolean AutoDrop) {
        automatic = AutoDrop;
    }

    public void SetLevel(int LevelSpeed) {
        level = LevelSpeed;
    }

    protected void handleFigureLandedHook() {
        //overwriteme
    }

    public void PrintGameResultHere(TetrisWriter tw, String floc) {
        tw.AppendToFile("Score:\t" + score, floc);
        String weights = "";
        for (int i = 0; i < myWS.getMyCO().getWeights().length; i++) {
            weights += ", " + myWS.getMyCO().getWeights()[i];
        }
        tw.AppendToFile("Weights:\t" + weights, floc);
        tw.AppendToFile("NeuralN:\t" + myWS.getMyCO().getMyNNCode(), floc);
        tw.AppendToFile("careFrequency, careHoleMade, firstPos:\t" + myWS.getMyCO().isCareIfTopIsnotFrequent() +", "
                + myWS.getMyCO().isCareIfHoleIsMadeByPlacement() + ", "+ myWS.getMyCO().getTakeFirstPositive(), floc);
        tw.AppendToFile("Back Out %:\t" + myWS.getMyCO().getmyPercentHigherRequirement(), floc);
    }
...
//    }
    ...
    public boolean GoYourFastest = false;
...
//            //addition here
//
//            //GetInstance().SetWeightSelector(WeightSelector.Constructor(CO));
//            //CO.setGame(GetInstance());
////            myWS = new WeightSelector(CO);
////            myWS.setVisible(true);
//            Button ShowController = new Button("Show Controller");
//            ShowController.setBackground(Configuration.getColor("button", "#d4d0c8"));
//            c = new GridBagConstraints();
//            c.gridx = 1;
//            c.gridy = 3;
//            c.weightx = 0.3;
...
//            c.anchor = GridBagConstraints.NORTH;
//            c.fill = GridBagConstraints.HORIZONTAL;
//            c.insets = new Insets(105, 100, 100, 100);
//            this.add(ShowController, c);
//            ShowController.addActionListener(new ActionListener() {
//                @Override
...
////                CO.setGame(GetInstance());
            if (REVERSIBLE) {//GetClassInstance() == TrainFromFailSequenceTetris.class || GetClassInstance() == TrainFromRegularRunning.class) {
                ...
                try {
                    ...
                    ((AbstractGame) GetInstance()).label = counterLabel;

                } catch (Exception e) {
                    JOptionPane.showMessageDialog(component, "Something failed. I think you tried to cast to abstractGame.");
                }
                ...
                oopsButton.addActionListener(new ActionListener() {
                    ...
                    protected void handleOopsButtonPressed() {
                        try {
                            ...
                            JOptionPane.showMessageDialog(component, "Not Available in this context.");
                        }
...
//        System.out.println("calling movePiece!!!!");
        ...
        movePieceHelper(best, pcp.getLeftplace());
    ...
    public void movePieceHelper(Cenario cenario, int j) {
        int rot = figure.getRotation();

        while (cenario.getMyfig().getOrientation() != rot) {
            if (rot < cenario.getMyfig().getOrientation()) {
                //create some sort of delay
                figure.rotateClockwise();
        ...
        int start = 5;
//         if (!figure.isAttached()) {
//             System.out.println("figure was not attached. I'll take care of it");
//             figure.attach(board, false);
//         }
        while (start != j) {
//            System.out.println("start:" + start + ", j: " + j);
            if (start > j) {
//                System.out.println("about to move left one");
                figure.moveLeft();
                start--;
            } else {
                figure.moveRight();
