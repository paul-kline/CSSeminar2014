...
public class TetrisWriter {
    ...
    public String GetTimeStamp(boolean month, boolean day, boolean year, boolean hour, boolean minute, boolean second) {
        ...
        if (day) {
            returnStr += (cal.get(Calendar.DAY_OF_MONTH)) + "_";
        }
        if (year) {
            returnStr += (cal.get(Calendar.YEAR)) + "____";
        }
        if (hour) {
            returnStr += (cal.get(Calendar.HOUR_OF_DAY)) + "_";
        }
        if (minute) {
            returnStr += (cal.get(Calendar.MINUTE)) + "_";
        }
        if (second) {
            returnStr += (cal.get(Calendar.SECOND)) + "_";
        }
...
//                + "_" + cal.get(Calendar.MINUTE) + "___" + cal.get(Calendar.SECOND);
    ...
    public synchronized boolean RewriteFileToThis(String newText, String location) {
        FileWriter fooWriter = null; 
        try {
            fooWriter = new FileWriter(location, false);
            // false to overwrite.
            fooWriter.write(newText);
            fooWriter.close();
            return true;
        } catch (IOException ex) {
            Logger.getLogger(TetrisWriter.class.getName()).log(Level.SEVERE, null, ex);
            JOptionPane.showMessageDialog(null, ex + "\n Error re-writing file.");
            return false;
        } 
